...
`/osb`, distributed as part of the portable `osb/` package. Generated, host-native entry
points (`.claude/`, `.codex/`, `.github/`) bind models and launch mechanics onto this
workflow — never redefine it, and are regenerated from `osb/hosts/` templates. Role agents
are self-contained and do not read this file or its references at dispatch time. All paths
below are workspace-root-relative (e.g. `osb/references/workflow.md`), not relative to
this file's own directory — this file is reproduced verbatim at more than one location for
host discovery.
...
See `osb/references/quality.md` §Completion gate for the full, binding checklist. In short:
...
- `osb/references/workflow.md` — full lifecycle detail, checkpoints, and control flow.
- `osb/references/roles.md` — responsibilities, prohibitions, and output schemas per role.
- `osb/references/handoff.md` — unit capsules, dispatch briefs, and delta-only repair loops.
- `osb/references/knowledge.md` — incremental knowledge format, storage, watermark, and consolidation.
- `osb/references/ragmonk.md` — RagMonk verification, retrieval budgets, and refresh policy.
